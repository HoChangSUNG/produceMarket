...
public class OrderProductQueryRepositoryTemp {
    ...
    public List<SellerTotalPricePerPeriodDTO> findAllSellerTotalPricePerPeriodAndCategory(YearMonth startDate, YearMonth endDate, CategoryParamDTO categoryParamDTO) {//기간별 판매액 비교(카테고리별 전체 판매자 기간별 총 금액)
...
//                "    where opp.status = 'ORDER' :categoryParam) op" +
...
//                " order by month_year_tb.ym";
        ...
        return jpaResultMapper.list(query, SellerTotalPricePerPeriodDTO.class);
    }

    public List<SellerTotalPricePerPeriodDTO> findSellerTotalPricePerPeriodAndCategory(YearMonth startDate, YearMonth endDate, CategoryParamDTO categoryParamDTO, Long userId) {//기간별 판매액 비교(카테고리별 특정 판매자 기간별 총 금액)
        String sql = "select cast(ifNull(sum(op.price * op.count),0) as SIGNED) as totalPrice, month_year_tb.ym as date" +
                "from (select opp.price as price, opp.count as count, o.created_date as created_date " +
                "    from order_product opp " +
                "    join orders o on opp.order_id = o.order_id " +
                ...
                "    join item i on k.item_code = i.item_code" +
                "    join item_category ic on i.item_category_code = ic.item_category_code" +
                "    where opp.status = 'ORDER'and u.user_id =:userId :categoryParam) op" +
                " right outer join  year_month_data" +
                " as month_year_tb on date_format(op.created_date,'%Y-%m') = month_year_tb.ym" +
        ...
        Query query = em.createNativeQuery(sql)
                ...
                .setParameter("categoryParam", categoryDynamic(categoryParamDTO));
